...
df['ATR'] = ta.ATR(df['High'].values, df['Low'].values, df['Settle'].values, timeperiod=14) # 1
df['ADX'] = ta.ADX(df['High'].values, df['Low'].values, df['Settle'].values, timeperiod=14) # 2
df['RSI'] = ta.RSI(df['Settle'].values, timeperiod=14) # 3
df['SMA'] = ta.SMA(df['Settle'].values, timeperiod=30) # 4
...
# Split data
predictors_list = ['ATR', 'ADX', 'RSI', 'ClgtEMA12', 'ClgtEMA26', 'MACDSIGgtMACD']
...
def generate_api_output():
    # Assuming the variables today and is_today_in_states are defined
    today = datetime.date.today()
    is_today_in_states = True  # Example value, replace this with your actual calculation

    # Check if today is the last date in df_filtered
    if today == last_date_in_df_filtered:
        action = "Buy"
        
        # Check if today is in filtered_states
        if is_today_in_states:
            secondary_action = "Sell"
        else:
            secondary_action = "Wait to Sell"
    else:
        action = "Wait to Decide"
        secondary_action = ""

    # Create a dictionary to represent the data
    data = {
        "today": str(today),
        "is_today_in_states": is_today_in_states,
        "action": action,
...
def save_to_json(data, filename="get_api.json"):
...
# Save API output to JSON file
save_to_json(api_output)
